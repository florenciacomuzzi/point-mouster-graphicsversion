...
* **MenuButtons** 
	* This script is used to determine where the player advances to, based on the button that triggered this script. It contains a `currLevel` int: to keep track of the current level.
...
* **PlayerController** 
	...
	* It contains many members since it controls a lot of technicalities of the player: a `moveSpeed` float, specifies the speed the player should move at.`jumpSpeed` float, specifies how  high the player should jump. `numBooks` int, the number of scientists/facts the player collected. `maxBooks` int, the total number of scientists/facts. `numBooksCollected` Text object, to display to the player so they see the progress. `myRigidBody` Rigidbody2D, used so the player can move and jump. `myAnim` Animator, used to set values to cause player animations.`facingRight` bool, used to keep the player facing to the right. `respanPosition` Vector3, used to determine the position where the player will respawn. `groundCheck` Transform, used to check if the player is on the 'ground'. `groundCheckRadius`float, which is the radius of the ground check space. `isGrounded` and `isJumping` bool, to know if the player is on the ground or jumping, `isPaused` to know if the player is in pause mode. `jumpSound, hurtSound, rightSound, wrongSound, collectSound` sounds to indicate when the player is jumping, hurt, answers a questions correct, answers a question incorrect, and when the player 'collects' a scientist. A `health` HealthBar object. A `bookNames` List, which contains a list of all the facts. And lastly, a `GAMetricIdx` enum, to specify the mode of play, the answer the player input, the level reached, the correct answer, the wrong answer and error.
	* All methods in this script are used to manupulate the above mentioned components.
